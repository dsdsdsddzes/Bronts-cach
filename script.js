...
function initDashboard() {
    ...
    loadTasksFromFirebase(currentUser.id, 'all');
    updateStatsFromFirebase(currentUser.id);
    ...
    document.getElementById('addTaskForm').addEventListener('submit', (e) => {
        ...
        push(ref(db, 'tasks/' + currentUser.id), task);
...
async function loadTasksFromFirebase(userId, filter) {
    try {
        const snapshot = await get(ref(db, 'tasks/' + userId));
        let tasks = [];
        if (snapshot.exists()) {
            const data = snapshot.val();
            tasks = Object.keys(data).map(key => ({ ...data[key], firebaseKey: key }));
        }
        renderTasksFromData(tasks, filter);
        return tasks;
    } catch (error) {
        console.error('Error loading tasks:', error);
        renderTasksFromData([], filter);
        return [];
    }
}

function renderTasksFromData(tasks, filter) {
    ...
    tasksList.innerHTML = filteredTasks.map(task => `
        <div class="task-card ${task.completed ? 'completed' : ''}" onclick="openTaskModal(${task.id})">
            <input type="checkbox" class="task-checkbox" ${task.completed ? 'checked' : ''} 
                   onclick="event.stopPropagation(); toggleTask(${task.id}, '${task.firebaseKey}')" />
...
function renderTasks(filter) {
    const currentUser = JSON.parse(localStorage.getItem('currentUser'));
    loadTasksFromFirebase(currentUser.id, filter);
}
...
async function updateStatsFromFirebase(userId) {
    try {
        const snapshot = await get(ref(db, 'tasks/' + userId));
        let tasks = [];
        if (snapshot.exists()) {
...
function toggleTask(taskId, firebaseKey) {
    const currentUser = JSON.parse(localStorage.getItem('currentUser'));
    const taskRef = ref(db, 'tasks/' + currentUser.id + '/' + firebaseKey);
    get(taskRef).then(snapshot => {
        if (snapshot.exists()) {
            const task = snapshot.val();
            update(taskRef, { completed: !task.completed });
            loadTasksFromFirebase(currentUser.id, getActiveFilter());
            updateStatsFromFirebase(currentUser.id);
        }
    }).catch(error => {
...
function openTaskModal(taskId) {
    const currentUser = JSON.parse(localStorage.getItem('currentUser'));
    get(ref(db, 'tasks/' + currentUser.id)).then(snapshot => {
        if (snapshot.exists()) {
            const data = snapshot.val();
            const firebaseKey = Object.keys(data).find(key => data[key].id === taskId);
            const task = data[firebaseKey];
            if (task) {
                ...
                deleteBtn.onclick = () => deleteTask(firebaseKey);
...
function deleteTask(firebaseKey) {
    if (confirm('هل أنت متأكد من حذف هذه المهمة؟')) {
        const currentUser = JSON.parse(localStorage.getItem('currentUser'));
        remove(ref(db, 'tasks/' + currentUser.id + '/' + firebaseKey));
        closeModal();
        loadTasksFromFirebase(currentUser.id, getActiveFilter());
        updateStatsFromFirebase(currentUser.id);
    }
...
function initializeDemoData(currentUser) {
    ...
    get(ref(db, 'bankData/' + currentUser.id)).then(snapshot => {
        if (!snapshot.exists()) {
            ...
            set(ref(db, 'bankData/' + currentUser.id), bankData);
            ...
            demoTasks.forEach(task => push(ref(db, 'tasks/' + currentUser.id), task));
        }
    }).catch(error => {
...
function displayTransactions(currentUser) {
    get(ref(db, 'transactions/' + currentUser.id)).then(snapshot => {
        let userTransactions = [];
        if (snapshot.exists()) {
            const data = snapshot.val();
            userTransactions = Object.values(data).sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 5);
        }
...
function handleMoneyTransaction(currentUser) {
    ...
    get(ref(db, 'bankData/' + currentUser.id)).then(snapshot => {
        ...
        // Save updated bank data to Firebase
        update(ref(db, 'bankData/' + currentUser.id), bankData);
        ...
        // Add to transactions in Firebase
        push(ref(db, 'transactions/' + currentUser.id), transaction);
        ...
        console.error('Error handling money transaction:', error);
        showMoneyMessage('حدث خطأ في معالجة العملية', 'error');
    });
}
